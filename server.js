 ...
 *   POST /api/trademark/recommend
 *   POST /api/trademark/generate-names
 */
...
function validateSession(req) {
  ...
  const match = cookie.match(/zc_session=([a-f0-9]+)/);
...
function getClientIp(req) {
  return req.headers['x-real-ip'] || req.headers['x-forwarded-for'] || req.socket.remoteAddress || 'unknown';
}
...
async function handleBusinessLicense(body) {
  ...
  if (!image) return { status: 400, data: { error: 'BAD_REQUEST', message: '缺少 image 字段' } };
  if (!image.startsWith('data:image/')) return { status: 400, data: { error: 'BAD_REQUEST', message: 'image 必须是 data:image/... 格式' } };

  const imgInfo = analyzeDataUrl(image);
  if (!imgInfo) return { status: 400, data: { error: 'BAD_REQUEST', message: 'base64 解析失败' } };

  if (clientHash && clientHash !== imgInfo.sha256) {
    return { status: 400, data: { error: 'HASH_MISMATCH', message: `图片传输校验失败`, clientHash: clientHash.substring(0, 8), serverHash: imgInfo.sha256.substring(0, 8) } };
  }

  console.log(`[VISION] image: ${imgInfo.mime} ${imgInfo.rawSize}B sha256=${imgInfo.sha256Short}`);

  // Auto-rotate: no debug file write to /tmp
  try {
  ...
  try {
    ...
    if (e.message === 'TIMEOUT') return { status: 504, data: { error: 'UPSTREAM_TIMEOUT', message: 'MiniMax API 请求超时 (90s)' } };
  ...
  if (result.status === 429) return { status: 429, data: { error: 'RATE_LIMITED', message: 'API 调用频率超限，请稍后重试' } };
  ...
  if (!content) return { status: 502, data: { error: 'EMPTY_RESPONSE', message: '模型返回空响应' } };
  ...
  return {
    status: 200,
    data: { success: true, model: MODEL, imageHash: imgInfo.sha256Short, imageSize: imgInfo.rawSize, imageMime: imgInfo.mime, fields, validationErrors, shouldAutoFill, rawText: content.slice(0, 2000) },
  };
}
...
async function handleLlmExtract(body) {
  ...
  if (!type || !context) return { status: 400, data: { error: 'BAD_REQUEST', message: '缺少 type 或 context' } };
...
const server = http.createServer(async (req, res) => {
  ...
  if (url.pathname === '/api/auth/login' && req.method === 'POST') {
    if (!checkRateLimit(ip)) {
      const lockout = getRemainingLockout(ip);
      return sendJSON(res, 429, { error: 'RATE_LIMITED', message: `登录失败次数过多，请${lockout}秒后重试` });
    }
    try {
      ...
      if (hash === expectedHash) {
        ...
        return sendJSON(res, 200, { status: 'ok' }, {
          'Set-Cookie': `zc_session=${token}; HttpOnly; Secure; SameSite=Strict; Max-Age=1800; Path=/`
        });
      } else {
        return sendJSON(res, 401, { error: 'AUTH_FAILED', message: '密码错误' });
      }
    } catch (e) {
  ...
  // Smart trademark: recommend categories
  if (url.pathname === '/api/trademark/recommend' && req.method === 'POST') {
    try {
      ...
      if (result.status !== 200) return sendJSON(res, 502, { error: 'UPSTREAM_ERROR', message: '推荐失败' });
      const content = result.data?.choices?.[0]?.message?.content || '';
  ...
  // Smart trademark: generate names
  if (url.pathname === '/api/trademark/generate-names' && req.method === 'POST') {
    try {
      ...
      const prompt = '基于以下GitHub项目信息，生成至少20个商标名称候选，覆盖不同路线（中文、英文、谐音、缩写、造字等）。每个名称要：\n- 2-6字/字母\n- 有显著性\n- 匹配项目调性\n\n' + desc + '\n\n只返回JSON：{"names":[{"name":"名称","type":"中文/英文/谐音/缩写/造字","meaning":"含义（10字内）","route":"路线","distinctiveness_score":0-100初步显著性评分,"text_risk_score":0-100文字风险评分(越低越好),"category":推荐类别数字}]}';
      const result = await callMiniMaxChat({
        model: MODEL,
        messages: [
      ...
      if (result.status !== 200) return sendJSON(res, 502, { error: 'UPSTREAM_ERROR', message: '生成失败' });
...
server.listen(PORT, '127.0.0.1', () => {
  ...
  console.log(`  Auth: Session/Cookie (30min TTL, 5 attempts/5min)`);
});
